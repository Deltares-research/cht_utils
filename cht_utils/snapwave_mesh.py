...
class SnapWaveMesh:
    ...
    def from_quadtree(self, qtr, mask, file_name=None, index_file_name=None):
        ...
        for iface in range(nfaces):
            for k in range(4):
                if face_nodes[k, iface] >= 0:
                    face_nodes[k, iface] = index_snapwave_in_quadtree[
                        face_nodes[k, iface]
                    ]
                else:
                    # This is a triangle
    ...
    def plot(self, ax, edgecolor="k", facecolor="r", fill=True):
        ...
        self.network._mesh2d.mesh2d_face_nodes[itriangle, 3] = (
            self.network._mesh2d.mesh2d_face_nodes[itriangle, 0]
        )

        import matplotlib.patches as patches

        xface = self.network._mesh2d.mesh2d_node_x[
            self.network._mesh2d.mesh2d_face_nodes
        ]
        yface = self.network._mesh2d.mesh2d_node_y[
            self.network._mesh2d.mesh2d_face_nodes
        ]
        ...
        for i in range(nfaces):
            ...
            p = patches.Polygon(
                path, facecolor=facecolor, edgecolor=edgecolor, fill=fill
            )
            ax.add_patch(p)
