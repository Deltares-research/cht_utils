...
def get_watersheds_in_bbox(gdb_path, output_path, bbox, layer, name):
    ...
    for j, watershed in enumerate(watersheds):
        if (
            watershed.bounds[0] < bbox[2]
            and watershed.bounds[2] > bbox[0]
            and watershed.bounds[1] < bbox[3]
            and watershed.bounds[3] > bbox[1]
        ):
            gs = watershed.geoms
            for polygon in gs:
...
def create_include_polygon(
    gdb_path, huc_file, output_path, name, layer="WBDHU8", dbuf=0.0, crs=None
):
    ...
    if crs:
        ...
        transformer = pyproj.Transformer.from_crs(wgs84, crs1, always_xy=True).transform
...
def main():
    ...
    parser.add_argument("wpath", type=str, help="GDB path with WBD database.")

    parser.add_argument(
        "-i", "--idfile", type=str, help="File with list of IDs to be merged."
    )
    ...
    parser.add_argument(
        "-n", "--name", type=str, metavar="name", default="wbd", help="Output name."
    )
    ...
    if args.command == "get":
        get_watersheds_in_bbox(
            args.wpath, args.outputpath, args.bbox, args.layer, args.name
        )
    elif args.command == "merge":
        create_include_polygon(
            args.wpath,
            args.idfile,
            args.outputpath,
            args.name,
