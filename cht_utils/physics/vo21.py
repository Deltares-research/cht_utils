...
class runup_vo21:
    ...
    def compute_setup(self, drspr, phi):
        ...
        v = (
            ...
            * (
                beta[1]
                + np.exp(-beta[2] * self.ksis ** beta[6] * ksib ** beta[3])
                * ksib ** beta[4]
            )
        )
    ...
    def compute_hm0_lf(self, drspr, phi):
        ...
        ksibm = (
            beta[4] * self.surfslope ** beta[3]
        )  # should increase with surfslope ie beta[4]>0
        cb = beta[2] * np.sqrt(self.surfslope)  # assume linear decrease ie beta[2]<0
        psibd = beta[0] * ksib ** beta[1]
        psibr = (beta[0] * ksibm ** beta[1] - 2.0) * (
            np.exp(-(ksib - ksibm) / (beta[2]))
        ) + 2
        psib = psibd
    ...
    def compute_hm0_hf(self, drspr, phi):
        ...
        self.hm0_hf = (
            ...
            * self.hm0
            * ksib ** beta[1]
            * np.tanh((self.ksis + beta[4]) ** beta[5] / (beta[2] * ksib ** beta[3]))
        )
    ...
    def compute_tm01_ig(self):
        ...
        tm0_ig = (
            beta[0]
            + beta[1] * self.surfslope ** beta[2] * self.steepness ** beta[3]
            + beta[4] * self.surfslope
        )
    ...
    def compute_ig_in(self):
        ...
        return self.hm0 * (
            beta[0]
            * self.surfslope ** beta[2]
            * np.exp(-beta[5] * self.surfslope ** beta[1])
            + beta[6] * np.exp(-beta[3] * self.steepness ** beta[4])
        )  # self.IG
    ...
    def compute_dirspreadfac_setup(self, drspr):
        ...
        return np.exp(
            -beta[0] * drspr ** beta[1] * (1.0 - np.tanh(beta[2] * self.ksis))
        )
