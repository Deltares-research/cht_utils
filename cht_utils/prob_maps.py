...
def merge_nc_his(file_list, variables, prcs=None, delete=False, output_file_name=None):
    ...
    for v in variables:
        ds[v] = xr.DataArray(
            data=np.zeros(
                tuple(
                    ds.dims[dim] if dim in ds.dims else nens for dim in new_dimensions
                )
            ),
            dims=new_dimensions,
...
def merge_nc_map_1by1(
    file_list, variables, prcs=None, delete=False, output_file_name=None, nbins=10
):
    ...
    i0 = np.zeros(
        (len(ds.timemax), np.shape(ds.x)[0], np.shape(ds.x)[1]), nbins
    ).astype(int)
    ...
    for v in variables:
        minstr = v + "_min"
        maxstr = v + "_max"
        ds[minstr] = xr.DataArray(
            data=d0 - 999.0,
            dims=("timemax", "n", "m"),
            coords={"timemax": ds.timemax, "x": ds.x, "y": ds.y},
        )
        ds[maxstr] = xr.DataArray(
            data=d0 + 999.0,
            dims=("timemax", "n", "m"),
            coords={"timemax": ds.timemax, "x": ds.x, "y": ds.y},
        )
